...
function CRMPageContent() {
  ...
  // Verificar más frecuentemente cuando el modal del QR está abierto
  useEffect(() => {
    if (showWhatsAppQRModal && qrCode) {
      const checkStatus = async () => {
        try {
          ...
          if (response.data.connected && !whatsappConnected) {
            setShowQRSuccessAnimation(true);
            setWhatsappConnected(true);
            setTimeout(() => {
              setShowWhatsAppQRModal(false);
              setShowQRSuccessAnimation(false);
              setQrCode(null);
            }, 2000);
          }
        } catch (error) {
  ...
  }, [showWhatsAppQRModal, qrCode, whatsappConnected]);

  // Función para obtener el código QR de WhatsApp
  const getWhatsAppQRCode = async () => {
    ...
    try {
      ...
      setShowWhatsAppQRModal(false);
      setQrError('No se pudo generar el código QR. Intentá de nuevo.');
    } catch (error) {
      setShowWhatsAppQRModal(false);
      setQrError('Error al obtener el código QR. Intentá de nuevo.');
    } finally {
      setQrLoading(false);
    }
  };
