            ...
            <div className="md:hidden space-y-2">
              {data.map((order) => (
                <div
                  ...
                  className="bg-white dark:bg-zinc-800 rounded-lg border dark:border-zinc-700 p-3 cursor-pointer active:bg-gray-50 dark:active:bg-zinc-700"
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="text-xs text-gray-500">
                      {formatDate(new Date(order.createdAt), locale, { day: 'numeric', month: 'short' })}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      <CategoryBadge category={order.category} locale={locale} />
                      <span className="text-sm font-medium">{order.totalItems || order.items?.length || 0} {t('products.units')}</span>
                    </div>
                  </div>
                  {(order.user?.name || order.vendor) && (
            ...
            <Table className="hidden md:table">
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{locale === 'es' ? 'Vendedor' : 'Seller'}</TableHead>
              ...
              <TableBody>
                {data.map((order) => (
                  ...
                  >
                    <TableCell>#{order.code || order.id?.slice(-6).toUpperCase()}</TableCell>
                    <TableCell>
                      <CategoryBadge category={order.category} locale={locale} />
                    </TableCell>
                    <TableCell>
