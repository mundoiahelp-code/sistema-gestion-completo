...
export default function StoreEdit({
  ...
  useEffect(() => {
    // Inicializar estado de días habilitados
  ...
  const handleOnOpen = (bol: boolean) => {
    setFormData(data);
    setOpen(bol);
  };
  ...
  return (
    <Dialog onOpenChange={handleOnOpen} open={open}>
      <DialogTrigger asChild>
        <Button variant={'link'} className='h-full w-full'>
          <FilePen className='stroke-1' />
        </Button>
      </DialogTrigger>
        ...
        <Tabs defaultValue='general' className='w-full'>
          ...
          <TabsContent value='turnos' className='space-y-4 mt-4'>
            ...
            <div className='bg-zinc-50 p-3 rounded-lg'>
              <p className='text-sm font-medium mb-2'>Vista previa de horarios disponibles</p>
              <p className='text-xs text-zinc-500'>
                Con turnos de {formData.appointmentDuration || 15} min y horario 09:00-19:00:
              </p>
              <p className='text-xs text-zinc-600 mt-1'>
                09:00, 09:15, 09:30, 09:45, 10:00... hasta 18:45
              </p>
            </div>
          </TabsContent>
        </Tabs>
