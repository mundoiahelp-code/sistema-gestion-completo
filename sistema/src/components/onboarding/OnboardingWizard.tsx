...
export default function OnboardingWizard({ onComplete }: { onComplete: () => void }) {
  ...
  const addStore = () => {
    setData(prev => ({
      ...prev,
      stores: [...prev.stores, {
        name: `Sucursal ${prev.stores.length + 1}`,
        address: '',
        phone: '',
        mondayHours: '09:00-19:00',
        tuesdayHours: '09:00-19:00',
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {isSpanish ? 'Configurá tus puntos de venta' : 'Configure your store locations'}
              </p>
            </div>
          </div>
                ...
                <div className="space-y-3">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      placeholder={isSpanish ? 'Sucursal Centro' : 'Downtown Store'}
                    />
                  </div>

                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-2">
                      <Label>{isSpanish ? 'Dirección' : 'Address'}</Label>
                      <Input
                        value={store.address}
                        onChange={(e) => updateStore(index, 'address', e.target.value)}
                        placeholder={isSpanish ? 'Av. Principal 123' : '123 Main St'}
                      />
                    </div>
                    <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    <Label className="flex items-center gap-2">
                      ...
                      {isSpanish ? 'Horarios' : 'Business Hours'}
                    </Label>
                    <div className="grid gap-2 text-sm">
                      {[
                        { key: 'mondayHours', label: isSpanish ? 'Lunes' : 'Monday' },
                        { key: 'tuesdayHours', label: isSpanish ? 'Martes' : 'Tuesday' },
                        { key: 'wednesdayHours', label: isSpanish ? 'Miércoles' : 'Wednesday' },
                        { key: 'thursdayHours', label: isSpanish ? 'Jueves' : 'Thursday' },
                        { key: 'fridayHours', label: isSpanish ? 'Viernes' : 'Friday' },
                        { key: 'saturdayHours', label: isSpanish ? 'Sábado' : 'Saturday' },
                        { key: 'sundayHours', label: isSpanish ? 'Domingo' : 'Sunday' },
                      ].map(({ key, label }) => (
                        <div key={key} className="flex items-center gap-2">
                          <span className="w-20 text-muted-foreground">{label}:</span>
                          <Input
                            value={store[key as keyof typeof store] as string}
                            onChange={(e) => updateStore(index, key, e.target.value)}
                            placeholder={isSpanish ? '09:00-19:00 o vacío si cerrado' : '09:00-19:00 or empty if closed'}
                    ...
                    <p className="text-xs text-muted-foreground">
                      {isSpanish 
                        ? 'Formato: HH:mm-HH:mm (ej: 09:00-19:00). Dejá vacío si está cerrado ese día.'
                        : 'Format: HH:mm-HH:mm (e.g: 09:00-19:00). Leave empty if closed.'}
                    </p>
                  </div>
                </div>
              </Card>
