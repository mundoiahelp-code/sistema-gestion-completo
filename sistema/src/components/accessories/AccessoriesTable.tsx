...
import MobileAccessoryCard from './MobileAccessoryCard';
...
function AccessoryInfo({ item, onUpdate }: { item: any; onUpdate: () => void }) {
  ...
  const fetchStores = async () => {
    ...
    try {
      ...
      const otherStores = res.data.filter((s: any) => s.id !== item.storeId);
    ...
    <div>
      ...
      {filteredData.length > 0 ? (
        <>
            ...
            {filteredData.map((item) => {
              ...
              return (
                <div 
                  key={item.id} 
                  className={twMerge(
                    'bg-white dark:bg-zinc-900 rounded-lg border dark:border-zinc-800 p-3',
                    hasReserved && 'border-green-500 bg-green-50/50 dark:bg-green-900/10'
                  )}
                >
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex-1">
                      <h3 className={twMerge('font-semibold text-sm', hasReserved && 'text-green-600')}>
                        {item.name}
                      </h3>
                      {item.model && <p className="text-xs text-muted-foreground">{item.model}</p>}
                    </div>
                    <span className={twMerge('text-lg font-bold', hasReserved ? 'text-green-600' : 'text-green-700')}>
                      ${item.price?.toLocaleString()}
                    </span>
                  </div>
                  <div className="flex items-center gap-3 mb-2 text-xs">
                    <span className={`px-2 py-0.5 rounded ${item.stock > 5 ? 'bg-green-100 text-green-700' : item.stock > 0 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}>
                      {item.stock} {t('products.units')}
                    </span>
                    {reserved > 0 && (
                      <span className="bg-green-200 text-green-800 px-2 py-0.5 rounded">{reserved} res.</span>
                    )}
                  </div>
                  {item.stockByStore && item.stockByStore.length > 0 && (
                    <div className="flex flex-wrap gap-1 mb-2">
                      {item.stockByStore.map((store: any, i: number) => (
                        <span key={i} className="text-xs bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-1.5 py-0.5 rounded">
                  ...
                  <div className="flex justify-end gap-1 pt-2 border-t">
                    <AccessoryInfo item={item} onUpdate={handleUpdate} />
                    <Button variant="ghost" size="sm" className="text-blue-600 hover:text-blue-800 hover:bg-blue-50 h-8 w-8 p-0" onClick={() => handleQuickSell(item)}>
                      <ShoppingCartIcon className="h-4 w-4" />
                    </Button>
                    <EditAccessoryDialog item={item} onUpdate={handleUpdate} />
                  </div>
                </div>
              );
            })}
          ...
          {/* Vista Desktop - Tabla */}
          <div className="hidden md:block rounded-md border bg-white dark:bg-zinc-900 dark:border-zinc-800 overflow-x-auto">
            <Table className="w-full">
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{t('accessories.stockByStore')}</TableHead>
                  <TableHead className="text-center">{t('accessories.reserved')}</TableHead>
                ...
                {filteredData.map((item) => {
                  ...
                  return (
                    <TableRow key={item.id} className={hasReserved ? 'bg-green-50 dark:bg-green-900/20' : ''}>
                      ...
                      <TableCell>
                        {item.stockByStore && item.stockByStore.length > 0 ? (
                          <div className="flex flex-wrap gap-1">
                            {item.stockByStore.map((store: any, i: number) => (
                              <span key={i} className="text-xs bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded">{store.stock} {store.storeName.substring(0, 3)}</span>
                            ))}
                          </div>
                      ...
                      <TableCell className={twMerge('text-green-700 font-semibold', hasReserved && 'text-green-500')}>${item.price?.toLocaleString()}</TableCell>
