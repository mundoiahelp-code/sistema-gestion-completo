    ...
    <div className="col-span-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center mb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={() => router.push('/ventas/crear')}>
      ...
      {/* Tabla Desktop */}
      <div className="bg-white dark:bg-zinc-900 rounded-lg border dark:border-zinc-800 hidden md:block">
        <Table>
          <TableHeader>
            <TableRow className="bg-gray-50 dark:bg-zinc-800">
              <TableHead>{t('sales.code')}</TableHead>
              <TableHead>{t('sales.seller')}</TableHead>
              <TableHead className="text-center">{t('sales.items')}</TableHead>
              {!isSeller && <TableHead>{t('common.total')}</TableHead>}
      ...
      <div className="md:hidden space-y-2">
        {filteredData.length === 0 ? (
          <div className="bg-white dark:bg-zinc-900 rounded-lg border dark:border-zinc-800 p-6 text-center text-gray-500 dark:text-gray-400">
            {t('sales.noSales')}
          </div>
        ) : (
          filteredData.map((item, i) => (
            <div
              key={i}
              className={twMerge(
                'bg-white dark:bg-zinc-900 rounded-lg border dark:border-zinc-800 p-3 cursor-pointer active:bg-gray-50 dark:active:bg-zinc-800',
                item.cancelled ? 'opacity-50' : ''
              )}
              onClick={() => router.push(`/ventas/${item.id}`)}
            >
              <div className="flex justify-between items-start mb-1">
                ...
                <span className="text-xs text-gray-500">
                  {formatDate(item.createdAt, locale, { day: 'numeric', month: 'short' })}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <div>
                  <p className="font-medium text-sm">{item.user?.name || (locale === 'es' ? 'Sin usuario' : 'No user')}</p>
