...
export default function TodayAppointments() {
  ...
  const fetchUpcomingAppointments = async () => {
    try {
      ...
      // Combinar, filtrar cancelados y ordenar
  ...
  const confirmCompleted = async () => {
    ...
    try {
      const token = Cookies.get('accessToken') || Cookies.get('token');
      await axios.patch(`${API}/appointments/${selectedAppointment.id}/status`, 
        { 
          status: 'COMPLETED',
  ...
  // Próximo turno (el primero que no pasó y no está completado)
  const nextAppointment = appointments.find(a => 
    !isPast(a.time) && a.status !== 'COMPLETED'
  );
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-4 mb-3 text-sm">
            ...
            <span className="text-green-500">
              {appointments.filter(a => a.status === 'COMPLETED').length} atendidos
            </span>
            <span className="text-yellow-500">
              {appointments.filter(a => a.status !== 'COMPLETED').length} pendientes
            </span>
          </div>
          ...
          <div className="space-y-2 max-h-[280px] overflow-y-auto pr-1">
            {appointments.map((apt: any) => {
              ...
              const past = apt.isToday && isPast(apt.time) && apt.status !== 'COMPLETED';
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-center">
                        ...
                        <div className={`text-lg font-mono font-bold ${
                          ...
                          apt.status === 'COMPLETED' ? 'text-zinc-400' :
                          past ? 'text-red-500' : 
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className={`font-medium text-sm ${
                            apt.status === 'COMPLETED' ? 'text-zinc-400' : 'text-zinc-900 dark:text-white'
                          }`}>
                            {apt.customerName}
                          </span>
                          {isNext && (
                    ...
                    {/* Acciones */}
                    {apt.status !== 'COMPLETED' && apt.isToday && (
                      <Button
                        size="sm"
                        variant="ghost"
