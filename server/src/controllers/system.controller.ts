...
const defaultConfig = {
  ...
  smtp: {
    ...
    from: process.env.SMTP_FROM || 'MundoIAple <sistema@mundoiaple.com>',
  },
};
...
export const testEmail = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await transporter.sendMail({
      ...
      subject: '✅ Test de Email - MundoIAple Sistema',
...
export const previewEmail = async (req: Request, res: Response) => {
  try {
    ...
    const systemName = config?.system?.systemName || 'MundoIAple';
...
export const setupSystem = async (req: Request, res: Response) => {
  try {
    ...
    const hashedPassword = await bcrypt.hash('lautimundoiaplezonasur', 10);
    ...
    res.json({
      ...
      credentials: {
        email: 'sistema@mundoiaple.com',
        password: 'lautimundoiaplezonasur'
      }
    });
